...
- The first generator command token must resolve unambiguously to a supported
  source below `generators/`. It may use the repo-relative path, the path below
  `generators/`, or an unambiguous basename, with or without its extension.
