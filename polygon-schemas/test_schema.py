...
def _warnings_completeness(root: Path) -> list[str]:
    ...
    if not build_path.exists():
        ...
        if isinstance(build, dict):
            if not build.get("accepted_solution_source"):
                warnings.append("config/build.json: accepted_solution_source is empty — no accepted solution configured")
            if not build.get("validator_source"):
                warnings.append("config/build.json: validator_source is empty — no validator configured")
    ...
    if not spec_path.exists():
        warnings.append("tests/spec.json: file missing — no tests defined")
    else:
        try:
        ...
        if isinstance(tests, list):
            if not tests:
                warnings.append("tests/spec.json: tests array is empty — no tests defined")
            else:
    ...
    if not sections_dir.is_dir():
        warnings.append("statement-sections/: directory missing — no statement languages")
    else:
...
def _warnings_judging_time(root: Path) -> list[str]:
    ...
    if total >= 300:
        return [f"this problem may take a fairly long time to judge ({tl_sec:.0f}s × {pass_limit} pass × {len(tests)} tests = {total:.0f}s)"]
    return []
...
def main() -> int:
    ...
    if not errors:
        print(f"OK — all schema checks passed ({root.name})")
        return 0
    for err in errors:
